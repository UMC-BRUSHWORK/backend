...
export const addReviewDB = async (data) => {
    try{
        ...
        // 필수 정보
        if(!(review_context || review_rate)) {
            throw new BaseError(status.INFO_NOT_EXIST);
            return -1;
        }

        if(getReviewByReviewId){
            return -1;
        }

        const result = await pool.query(insertReviewSql, [body.review_id, body.review_product, body.review_context, body.review_rate, created_at]);
