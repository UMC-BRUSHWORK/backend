...
// 약관 동의 수정
tosRouter.patch('/:utId', jwtAuthenticationMiddleware, asyncHandler(tosOptionController));

// 약관 리스트 전달 (위는 단순 약관(tos), 아래는 사용자 약관(user_tos))
tosRouter.get('/list', asyncHandler(tosListController));
tosRouter.get('/:userId', jwtAuthenticationMiddleware, asyncHandler(tosUserListController));